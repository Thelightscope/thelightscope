...
class Ports:
    def __init__(self,
        ...
        self.ARP_requests = collections.deque(maxlen=MAX_SIZE)
        self.ARP_same_timestamp = collections.deque(maxlen=MAX_SIZE)
        self.timer=0
...
def is_virtual_interface(interface_name):
    ...
    virtual_patterns = [
        ...
        'vlan.',       # VLAN sub-interfaces (e.g. vlan.100)
...
class configuration_reader:
    ...
    def initialize_config(self,config_file):
        ...
        # Check for the 'database' option. Priority order:
        # 1. Existing value in config file (a user-chosen custom db_id always wins)
        # 2. Environment variable LIGHTSCOPE_DB_NAME (used only to seed a fresh config,
        #    e.g. the value generated by the installer/systemd on first install)
        # 3. Generate a new one if neither exists
        existing_db = config['Settings'].get('database', '').strip()
        env_db_name = os.environ.get('LIGHTSCOPE_DB_NAME', '').strip()
        if existing_db and existing_db.lower() != 'uninitialized':
            # Respect the value already in config.ini so a custom db_id is never overwritten
            print(f"Using existing database name from config: {existing_db}")
        elif env_db_name:
            # Seed from the environment (systemd service) only when config has no value yet
            config['Settings']['database'] = env_db_name
            print(f"Using database name from environment: {env_db_name}")
        else:
            # Generate new database name only if neither config nor environment provides one
...
def _honeypot_worker(top_unwanted_ports_consumer, shared_open_honeypots, hp_upload_producer, external_network_information, config_settings, system_info, shared_honeypot_config):
    try:
        ...
        # Ensure all prints are immediately visible
        sys.stdout.flush()
        sys.stderr.flush()
        ...
        def prepare_honeypot_connection_data(sport, service, ipsrc,dport):
            ...
            ip_src = ipsrc                            # Source IP (attacker)
            ...
            # =============================================================================
            tcp_sport = str(sport)                  # Source port (attacker)
            tcp_dport = str(dport)                         # Destination port (honeypot service)
                        ...
                        for sock in ready:
                            try:
                                ...
                                if svc in ('SSH', 'TELNET'):
                                    ...
                                    # send PROXY header - format: PROXY TCP4 src_ip dest_ip src_port dest_port
                                    # Get the local IP that the client connected to
        ...
        print(f"_honeypot_worker: FATAL ERROR - Worker crashed: {e}", flush=True)
        import traceback
        traceback.print_exc()
        sys.stdout.flush()
        sys.stderr.flush()
